...
function apply(ctx) {
	...
	ctx.tools.register(defTool("list_sessions", "列出当前进程内存中所有\"活\"的对话（含当前对话），返回每条的会话 id 与状态（idle/running）。要发消息给另一个对话时，先调用本工具拿它的会话 id，再用 send_to_session 投递。注意：只有已加载/正在运行的对话会出现在这里；冷(未打开)的对话不在列表中。本工具列的是\"平级对话\"，不是 subagent 子代理。", {}, async () => {
	...
	ctx.tools.register(defTool("send_to_session", "把一条文本消息直接投递并唤醒另一个\"对话\"（跨对话调用，非 subagent）。目标对话收到后会在自己的独立上下文中立即开始新一轮处理。目标用会话 id 指定（先用 list_sessions 查）。消息会被当作一条普通 user 消息投送给对方。默认 expectReply=true：会自动在消息末尾附上\"完成后把结果回传给我\"的指令，因此对方完成后会主动回传，无需你手写回传要求。当你这条消息本身就是\"回传结果\"时，请设 expectReply=false，避免对方再回传（防止来回循环）。", {
		sessionId: {
			type: "string",
			required: true,
			description: "目标对话的会话 id（来自 list_sessions）。"
		},
		message: {
			type: "string",
			required: true,
			description: "要发送给对方对话的正文。"
		},
		expectReply: {
			type: "boolean",
			description: "是否要求对方完成后回传（默认 true）。回传结果时请设为 false。"
		}
	}, async (args, exec) => {
		...
		if (senderId !== void 0 && senderId !== "" && targetId === String(senderId)) return {
			ok: false,
			error: "不能给自己发消息（sessionId 等于当前对话，请用 list_sessions 重新确认目标 id）"
		};
		const target = agents.get(targetId);
		if (target === void 0) return {
			ok: false,
			error: "未找到该对话（可能离线/未加载，或 id 不对）：" + targetId
		};
		let deliver = text;
